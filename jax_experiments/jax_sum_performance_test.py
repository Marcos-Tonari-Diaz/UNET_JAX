...
if __name__ == "__main__":
    ...
    (f'sum: {perf_timer_jax(jax_random_sum)(rand_vec_jax)}')

    (f'sum: {perf_timer_jax(jax_random_sum_device_put)(rand_vec_jax)}')

    jax_random_sum_jit(dummy)  # discard first run
    (f'sum: {perf_timer_jax(jax_random_sum_jit)(rand_vec_jax)}')

    jax_random_sum_jit_device_put(dummy)  # discard first run
    (f'sum: {perf_timer_jax(jax_random_sum_jit_device_put)(rand_vec_jax)}')

    (f'sum: {perf_timer(numpy_random_sum_batched)(rand_vec_numpy)}')

    (f'sum: {perf_timer_jax(jax_random_sum_batched)(batched_arr)}')

    jax_random_sum_vmap(dummy)  # discard first run
    (f'sum: {perf_timer_jax(jax_random_sum_vmap)(batched_arr)}')

    jax_random_sum_vmap_jit(dummy)  # discard first run
    (f'sum: {perf_timer_jax(jax_random_sum_vmap_jit)(batched_arr)}')

    jax_random_sum_pmap(dummy)  # discard first run
    (f'sum: {perf_timer_jax(jax_random_sum_pmap)(batched_arr)}')
